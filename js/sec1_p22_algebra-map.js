...
var AlgCore = (function () {
  ...
  var RESIDENTS = [
    ...
    { id: 'PIDgap', label: '(gap)', tightest: 'pid', witnessKey: null, gapNote: true,
      blurb: 'Every Euclidean domain is a PID (proved on this page), and the converse is known to fail \u2014 but the classical witness lies beyond this page\u2019s scope.' },
  ...
  function runSelfTests() {
    ...
    (function () {
      ...
      var claims = {
        ...
        PIDgap: idx.pid, Z: idx.ed, F7x: idx.ed, F7: idx.field, Q: idx.field
      };
...
(function () {
  ...
  function init() {
    ...
    G.RESIDENTS.forEach(function (r) {
      ...
      if (r.gapNote) {
        var agN = 150 * Math.PI / 180;
        svg += '<text x="' + (300 + rad * Math.cos(agN)).toFixed(1) + '" y="' +
          (300 - rad * Math.sin(agN)).toFixed(1) + '" text-anchor="middle" class="alg-gapmark" ' +
          'data-resident="' + r.id + '">?</text>';
        return;
      }
    ...
    style.textContent =
      ...
      '#algebra-map-root .alg-layout{display:flex;flex-wrap:wrap;gap:16px;align-items:flex-start;}' +
      '#algebra-map-root .alg-mapcell{flex:1.1;min-width:290px;max-width:470px;}' +
      ...
      '#algebra-map-root .alg-gapmark{fill:' + C.faint + ';font-size:16px;font-weight:bold;cursor:pointer;}' +
      '#algebra-map-root .alg-info{flex:1;min-width:280px;border:1px solid ' + C.border + ';border-radius:8px;' +
    ...
    function showLayer(key) {
      ...
      bodyEl.innerHTML = L.desc + '<span class="alg-ex">Members include: ' + L.ex + '</span>';
      vecEl.innerHTML = '';
    ...
    function showResident(id) {
      ...
      nameEl.textContent = r.gapNote ? 'PID \u2229 (not ED): a real but distant gap' : r.label;
    ...
    root.querySelectorAll('.alg-dotgroup, .alg-gapmark').forEach(function (d) {
