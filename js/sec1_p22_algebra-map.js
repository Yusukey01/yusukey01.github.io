...
const renderAlgebraCompass = () => {
    ...
    // DATA: Validated Hierarchy with Dark-Theme Optimized Colors
    const structures = [
        ...
        { name: "Unique Factorization Domain",  r: 145, color: "#8e44ad", desc: "Unique Factorization Domain. Guaranteed unique 'prime' decomposition.", ex: "ℤ[x] (A UFD, but not a PID)" },
        { name: "Principal Ideal Domain",       r: 105, color: "#5f27cd", desc: "Principal Ideal Domain. Every ideal is generated by a single element.", ex: "ℤ, ℚ[x]" },
    ...
    root.innerHTML = `
        <style>
            .map-layer { cursor: pointer; transition: all 0.3s ease; stroke: rgba(255,255,255,0.2); stroke-width: 1; fill-opacity: 0.8; }
            .map-layer:hover { fill-opacity: 1; stroke: #00d2d3; stroke-width: 3; filter: drop-shadow(0 0 8px rgba(0,210,211,0.5)); }
            .compass-label { font-family: 'Inter', sans-serif; font-size: 14px; fill: #ecf0f1; font-weight: bold; pointer-events: none; text-transform: uppercase; letter-spacing: 1px; }
            .compass-center-label { font-family: 'Inter', sans-serif; font-size: 11px; fill: #fff; font-weight: bold; text-anchor: middle; pointer-events: none; }
            #structure-info-box { 
            ...
            #info-ex { color: #00d2d3; margin: 0; font-size: 0.9em; font-style: italic; }
        </style>
        ...
        <svg id="algebra-svg" viewBox="0 0 600 600" style="max-width: 500px; width: 100%; height: auto;">
            ...
            `).join('')}
            <text x="300" y="55" text-anchor="middle" class="compass-label">Groups</text>
            <text x="300" y="304" class="compass-center-label">FIELD</text>
        </svg>

        <div id="structure-info-box">
            <h3 id="info-name">Hierarchy of Integrity</h3>
            <p id="info-desc">Hover over the layers to explore how mathematical structures nest. In a dark universe, the Field shines the brightest as the most powerful structure.</p>
            <p id="info-ex"></p>
        </div>
    `;
